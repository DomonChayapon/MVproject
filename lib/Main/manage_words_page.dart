...
class _ManageWordsPageState extends State<ManageWordsPage> {
  ...
  void _addWord() {
    ...
    // สร้างรายการคำใหม่โดยใช้ push เพื่อสร้างกุญแจที่ไม่ซ้ำ
    String key = ref.child("Word").push().key!;
    ...
    // ใช้ push().set() เพื่อเพิ่มคำใหม่ลงในฐานข้อมูล
    ref.child("Word/$key").set(wordData).then((_) {
      print('Data added successfully'); // ข้อมูลถูกบันทึกสำเร็จ

      // อัปเดตรายการ _words ในเครื่องด้วย
      setState(() {
        _words.add(wordData);
        _wordController.clear(); // ล้างช่องกรอกข้อมูล
        _translationController.clear();
        _selectedType = null; // รีเซ็ตประเภทที่เลือก
      });
    }).catchError((error) {
      print('Error adding data: $error'); // จัดการข้อผิดพลาด
    });
  }
  ...
  void _removeWord(int index) {
    ...
    DatabaseReference ref =
        FirebaseDatabase.instance.ref("Vocabulary/$wordKey");
